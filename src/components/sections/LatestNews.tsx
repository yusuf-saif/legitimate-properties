        ...
        <div className="grid md:grid-cols-3 gap-6">
          {posts.map(post => (
            <Link key={post._id} href={`/news/${post.slug}`}
              className="group bg-white rounded-card shadow-card hover:shadow-card-hover overflow-hidden transition-shadow duration-300">
              <div className="relative aspect-[3/2] overflow-hidden">
                {post.featuredImage && (
                  <Image src={urlFor(post.featuredImage).width(600).height(400).url()} alt={post.featuredImage.alt ?? post.title}
