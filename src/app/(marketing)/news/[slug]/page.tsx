...
export default async function NewsDetailPage({ params }: Props) {
  const raw = await prisma.newsPost.findUnique({ where: { slug: params.slug } })
  if (!raw) notFound()
  const post = mapNewsPost(raw)

  const relatedRaw = await prisma.newsPost.findMany({
    where: { slug: { not: params.slug } },
    take: 3,
    orderBy: { publishedAt: 'desc' },
  })
  const relatedPosts = relatedRaw.map(mapNewsPost)
      ...
      <div>
        ...
        <section className="section-padding bg-white">
          <div className="container-lp max-w-text mx-auto">
            ...
            <div
              className="prose text-text-body [&_h2]:heading-h2 [&_h2]:text-espresso [&_h2]:mb-4 [&_h2]:mt-10 [&_p]:text-body-md [&_p]:mb-4 [&_p]:text-text-body"
          ...
          <section className="section-padding bg-cream">
            <div className="container-lp">
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {relatedPosts.map(relatedPost => <NewsCard key={relatedPost.id} post={relatedPost} />)}
              </div>
            </div>
          </section>
        )}
