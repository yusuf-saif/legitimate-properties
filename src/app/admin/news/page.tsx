  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="heading-h2 text-espresso">News Articles</h1>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-border-soft bg-cream/50">
              ...
              <th className="p-4 text-body-sm font-semibold text-espresso">Category</th>
              <th className="p-4 text-body-sm font-semibold text-espresso">Author</th>
              <th className="p-4 text-body-sm font-semibold text-espresso">Published</th>
              <th className="p-4 text-body-sm font-semibold text-espresso">Actions</th>
            </tr>
          </thead>
          <tbody>
            {posts.length === 0 ? (
              <tr><td colSpan={5} className="p-8 text-center text-text-muted text-body-sm">No articles yet.</td></tr>
            ) : posts.map(p => (
              <tr key={p.id} className="border-b border-border-soft hover:bg-cream/30 transition-colors">
                ...
                <td className="p-4 text-body-sm text-text-muted">{p.category}</td>
                <td className="p-4 text-body-sm text-text-muted">{p.author}</td>
                <td className="p-4 text-body-sm text-text-muted">{p.publishedAt.toLocaleDateString()}</td>
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    <Link href={`/admin/news/${p.id}/edit`}
                      className="p-1.5 text-text-muted hover:text-terracotta transition-colors">
                      <Edit size={16} />
                    </Link>
                    <DeleteButton id={p.id} type="news" />
                  </div>
                </td>
              </tr>
            ))}
